...
$original_select = 'SELECT c.*,(SELECT COUNT(ccc.fk_cid) FROM #__os_cck_categories_connect AS ccc <br>
                    INNER JOIN #__os_cck_entity_instance AS cei ON ccc.fk_eiid=cei.eiid <br>
                    INNER JOIN #__os_cck_layout AS ley ON ley.lid = cei.fk_lid <br>
                    WHERE ccc.fk_cid=c.cid AND ley.type = "add_instance" <br>
                    AND cei.fk_eid = "$allcategories_layout->fk_eid") AS items, 0 as display <br>
                    FROM  #__os_cck_categories AS c <br>
                    WHERE c.section="com_os_cck"<br>
            ...
            <div class="main-fields-content main-fields-options">
              ...
              if(isset($layout_params['views']['show_request_layout'])){
                foreach ($layout_params['views']['show_request_layout'] as $key => $value) {
                    if(isset($layout_params['views']['show_type_request_layout']) && !array_key_exists($key, $layout_params['views']['show_type_request_layout'])){ ?>
                        <select id="vi_show_type_request_layout<?php echo $key ?>" style="display: none" name="vi_show_type_request_layout[<?php echo $key ?>][]">
                                <option value="1" selected="selected">Form</option>
