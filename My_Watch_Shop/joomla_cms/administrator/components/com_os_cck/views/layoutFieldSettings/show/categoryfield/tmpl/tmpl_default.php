...
$show_type_selected = (isset($lParams['show_type_'.$fName])) ? $lParams['show_type_'.$fName] : "title";
$label_tags = array();
...
$layouts_for_link_options = get_options_for_layout_select_list('category', $field->fk_eid, $layout->lid);

$show_options = array();
$show_options[]  = JHTML::_('select.option','title','Title only');
$show_options[]  = JHTML::_('select.option','image-title','Image-Title');
$show_options[]  = JHTML::_('select.option','title-image','Title-Image');
$show_options[]  = JHTML::_('select.option','image','Image only');
...
<div id="options-field-<?php echo $fName?>">
    ...
    <div>
        <label><?php echo JText::_("COM_OS_CCK_LABEL_SHOW_TYPE_CATEGORYFIELD")?></label>
        <?php echo JHTML::_('select.genericlist',$show_options, 'fi_show_type_'.$fName,
                    'size="1" class="inputbox" ', 'value', 'text', $show_type_selected);?>
    </div>
    <div>
...
<script type="text/javascript">
    var field = 'fi_show_type_' + '<?php echo $fName; ?>';
    
    function showHideImageSetting(field){
        var show_type = jQuerOs('#'+field).val();
        
        if(show_type.indexOf('image') > -1){
            jQuerOs('.image_settings').show();
        }else{
            jQuerOs('.image_settings').hide();
        }
    }
    
    showHideImageSetting(field);
    
    jQuerOs('#'+field).on('change', function(){
        showHideImageSetting(field);
    });
</script>