...
$apply_button_type = (isset($fields_from_params['apply_button_type_'.$fName])) ? $fields_from_params['apply_button_type_'.$fName] : "button";
$apply_button_text = (isset($fields_from_params[$fName.'_apply_button_text'])) ? $fields_from_params[$fName.'_apply_button_text'] : '';
$icon_apply_coupon = (isset($fields_from_params[$fName.'_add_icon_apply_coupon'])) ? $fields_from_params[$fName.'_add_icon_apply_coupon'] : '';
...
$apply_button_type_opt = array();
$apply_button_type_opt[] = JHTML::_('select.option','button','Button');
$apply_button_type_opt[] = JHTML::_('select.option','icon','Icon');
...
<!--    <div>
    ...
    <div class="apply_coupone_wrap">
        <label><?php echo JText::_("COM_OS_CCK_LABEL_APPLY_COUPON_TYPE")?></label>
        <?php echo JHTML::_('select.genericlist',$apply_button_type_opt, 'fi_apply_button_type_'. $fName,
                            'size="1" class="inputbox" ', 'value', 'text', $apply_button_type);?>
    </div>
    
    <div class="apply_button_wrap">
        <label><?php echo JText::_("COM_OS_CCK_LABEL_APPLY_COUPON_BUTTON_TEXT")?></label>
        <input type="text" size="4" name="fi_<?php echo $fName?>_apply_button_text"  value="<?php echo $apply_button_text?>" >
    </div>
    
    <div class="apply_icon_wrap">
        <label><?php echo JText::_("COM_OS_CCK_LABEL_APPLY_COUPON_ICON")?></label>
        <div class="fa <?php echo $icon_apply_coupon; ?> add_font_awesom" id="<?php echo $fName;?>_add_icon_apply_coupon" rel="tooltip" data-toggle="tooltip" data-placement="top" title="<?php echo JText::_("COM_OS_CCK_TOOLTIP_ADD_ICON");?>"><?php echo ($icon_apply_coupon == '') ? '&#10000;' : ''; ?></div>
        <input type="hidden" name="fi_<?php echo $fName; ?>_add_icon_apply_coupon" value="<?php echo $icon_apply_coupon; ?>">
    </div>
...
<script type="text/javascript">
    function showHideApplyCouponType(){
        var type = jQuerOs('#fi_apply_button_type_coupon').val()
        //console.log('12121212121212', type)
        if(type == 'button'){
            jQuerOs('.apply_button_wrap').show()
            jQuerOs('.apply_icon_wrap').hide()
        }else if(type == 'icon'){
            jQuerOs('.apply_button_wrap').hide()
            jQuerOs('.apply_icon_wrap').show()
        }
    }
    
    showHideApplyCouponType();
    
    jQuerOs('#fi_apply_button_type_coupon').on('change', showHideApplyCouponType);
</script>
