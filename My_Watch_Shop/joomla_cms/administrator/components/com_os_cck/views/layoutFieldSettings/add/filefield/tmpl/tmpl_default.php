...
<div id="options-field-<?php echo $fName?>">
    ...
    <div>
        <label><?php echo JText::_("COM_OS_CCK_LABEL_SHOW_FIELD_REQUIRED")?> <i title="<?php echo JText::_("COM_OS_CCK_LABEL_SHOW_FIELD_REQUIRED_FILEFIELD_DESC")?>" class="glyphicon glyphicon-info-sign date_tooltip"></i></label>
    ...
    <?php if(isset($cck_entity_configuration[$field->fk_eid]['check_access_fields']) && $cck_entity_configuration[$field->fk_eid]['check_access_fields'] == '1'){ ?>
    <div>
...
<?php
if($allowed_values == 'on'){
    if($fld_require == ''){
        echo "<script>jQuerOs('[name=fi_".$fName."_required]').trigger('click'); </script>";
    }
    echo "<script>jQuerOs('[name=fi_".$fName."_required]').attr('disabled', 'disabled');</script>";
}