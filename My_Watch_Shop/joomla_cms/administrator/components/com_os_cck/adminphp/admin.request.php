...
class AdminRequest{
  ...
  static function showRequestItem($option, $bid, $recurse = 0){
    global $db, $my,$app;
    ...
    if(empty($parent_instance)){
        if($recurse == 1){
            ...
            $parent_instance = new os_cckLayout($db);
            if($parent_instance->getDefaultLayout($entityInstance->fk_eid, 'instance')){

                $parent_instance->load(intval($instance_layout->getDefaultLayout($entityInstance->fk_eid, 'instance')));
    ...
    AdminViewRequest :: showRequestItem($option, $entityInstance, $str_list, $recurse);
  }
