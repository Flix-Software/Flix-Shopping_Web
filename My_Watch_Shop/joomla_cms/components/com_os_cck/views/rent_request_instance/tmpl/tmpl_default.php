...
<div class="cck-body">
  ...
  <div <?php echo $div_styling; ?>>
    <!--<a <?php echo $button_style['field_styling'];?> class="button <?php echo $button_style['custom_class'];?>" <?php echo $button_style['offset_animation']; ?> href="<?php echo $link ?>">
      ...
      <form name="show_request_layout_<?php echo $rand; ?>" id="show_request_layout_<?php echo $rand; ?>" method="post" action="index.php" enctype="multipart/form-data" >
        ...
        <input type="button" class="button <?php echo $custom_class;?>" <?php echo $offset_animation; ?> <?php echo $field_styling;?> onclick="javascript:submit_show_request_layout('show_request_layout_<?php echo $rand; ?>')" value="<?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_RENT_REQUEST'); ?>">
    </form>
  </div>
  ...
  foreach ($fields_list as $value) {
    ...
    if($value->field_type == 'datetime_popup'){
      if(isset($field_from_params[$value->db_field_name.'_field_type']) == 'rent_from'){
        $field_from_capt = $value->db_field_name;
        $from = true;
      }
      if(isset($field_from_params[$value->db_field_name.'_field_type']) == 'rent_to'){
  ...
  if($show_type == 2){
     ...
     $offset_animation = isset($button_style['offset_animation']) ? $button_style['offset_animation'] : '';?>
  <div <?php echo $div_styling; ?>>
    <input <?php echo $field_styling;?> type="button" class="button btn btn-info <?php echo $custom_class;?>" <?php echo $offset_animation; ?> value="<?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_RENT_REQUEST'); ?>"
  ...
  ?>
  <form class="<?php echo $form_custom_class; ?>" <?php echo $form_styling; ?> <?php echo $form_offset_animation; ?> <?php echo $form_hover_animation; ?> action="index.php" method="post" name="<?php echo $unique_form_id?>" id="<?php echo $unique_form_id?>" enctype="multipart/form-data">
    <?php
...
//            }else {
    ...
    <input <?php echo $field_styling; ?> type="button" name="rent_request_button" value="<?php echo $buttonText; ?>"
           class="button <?php echo $custom_class; ?>" <?php echo $offset_animation; ?> onclick="javascript:sendRequest<?php echo $layout->type?>('send_rent_request',<?php echo $unique_form_id?>);">
    <?php    
    echo '</div>';
    ...
    <input type="hidden" name="eid" value="<?php echo $layout->fk_eid?>"/>
...
<script>
    ...
    function sendRequest<?php echo $layout->type?>(task,form_name){
      ...
      if(task == 'send_rent_request'){
        ...
        if(jQuerOs(form).find('[name=rent_from]') == undefined){
          alert("Please in RentRequest layout create(set) Date->RentFrom and Date->RentTo");
          return;
        }
        if(jQuerOs(form).find('[name=rent_until]') == undefined){
          alert("Please in RentRequest layout create(set) Date->RentFrom and Date->RentTo");
          return;
        }
        
        if(jQuerOs(form).find('[name=rent_from]').val() == ''){
          alert("Please enter a start date for the rental");
          return;
        }
        if(jQuerOs(form).find('[name=rent_until]').val() == ''){
        ...
        if('<?php echo  $os_cck_configuration->get('rent_type');?>' == 2){
          ...
          if(jQuerOs(form).find('[name=time_from]').val() == ''){
            alert("Please, choose time from");
            return;
          }
          if(jQuerOs(form).find('[name=time_until]').val() == ''){
