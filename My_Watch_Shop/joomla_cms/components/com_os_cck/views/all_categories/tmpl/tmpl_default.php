  ...
  function showAllCat($option, $id, $categories, $params, $Itemid, $deep,$layout,$layout_params) {
    ...
    for ($i = 0; $i < count($categories); $i++) { //
      ...
      if (($id == $categories[$i]->parent_id) && ($categories[$i]->display == 1)) {
        ...
        echo '<'.$shell_tag.' class="cat_instance_body'.$custom_class.'" ' . $offset_animation . ' ' .$field_styling.'>';
                ...
                jQuerOs( document ).ready(function() {
                    ...
                    var cckGridLayout1 = function (container, params) {
                        ...
                        // jQuerOs(container+" .item").css("width",imgBlockW+"%");
                            ...
                            jQuerOs(container+" .cat_instance_body").css({
                            'width': imgBlockW+"%",
                            ...
                            cckGrid.resizeGallery = function (){
                                imgBlockW = cckGrid.getImgBlockWidth();
                                jQuerOs(container+" .cat_instance_body").css("width",imgBlockW+"%");
                            }
           ...
           if($php_result){
            ...
            if(strpos($layout_html,"{|l-".$key."|}")){
              ...
              if(isset($layout_params['views']['show_request_layout'][$layout_params['key']])
                ...
                if(!checkAccess_cck($layout_params['views']['show_request_layout'][$layout_params['key']], $user->groups, $layout->fk_eid, 'fields')){
      ...
      foreach ($layout_params['attachedModule'] as $attachedModule) {
        if($attachedModule){
          if(strpos($layout_html,"{|m-".$attachedModule->id."|}")){
            ...
            $html = '<'.$shell_tag.' class="'.$custom_class.'" ' . $offset_animation . ' ' .$field_styling.'>'.JModuleHelper::renderModule($module,$options).'</'.$shell_tag.'>';
