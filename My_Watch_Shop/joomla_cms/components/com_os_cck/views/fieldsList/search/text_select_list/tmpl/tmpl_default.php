...
if($fParams['type'] == 2){?>
    ...
    <?php
    return;
}elseif($fParams['type'] == 0){ ?>
    <input type="hidden" name="os_cck_search_<?php echo $fName?>" value="<?php echo $layout_params['fields'][$fName.'_default_value']; ?>">
    <?php return;
}
$required = '';
...
if($value == ''){
    ...
    $value = $layout_params['fields'][$fName.'_default_value'];
}
...
echo JHTML::_('select.genericlist', $arr, $tag_name, $field_styling.' class="'.$custom_class.$required.'"'.$select_type . ' ' . $offset_animation, $key, $text, $value);
