...
<div class="cck-body">
  ...
  if(is_array($layout->parent_eiid) && empty($layout->parent_eiid)){
      $parent_eiid = '';
  }elseif(is_array($layout->parent_eiid) && !empty($layout->parent_eiid)){
      //foreach($layout->parent_eiid as $)
      $parent_eiid = implode(',', $layout->parent_eiid);
  }else{
      $parent_eiid = $layout->parent_eiid;
  }
  
  $unique_form_id = $layout_type.'_Form_'.$parent_eiid.$moduleId.rand();
  $unique_form_id = str_replace(',', '', $unique_form_id);
  //var_dump($unique_form_id);
  ...
  if($show_type == 3){
    ...
    $link = JRoute::_("index.php?option=com_os_cck&task=show_request_layout$link_eiid&lid=".$layout->lid."&Itemid=$Itemid");
    ...
    //$link = JURI::root()."index.php?option=com_os_cck&task=show_request_layout$link_eiid&lid=".$layout->lid."&Itemid=$Itemid";
    ?>
  ...
  if($show_type == 2){ 
     ...
     $offset_animation = isset($button_style['offset_animation']) ? $button_style['offset_animation'] : '';?>
  <div <?php echo $div_styling; ?>>
    <input <?php echo $field_styling;?> type="button" class="button btn btn-info <?php echo $custom_class;?>" <?php echo $offset_animation; ?> value="<?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_SEND_REQUEST'); ?>"
  ...
  ?>
  <form class="<?php echo $form_custom_class; ?>" <?php echo $form_offset_animation; ?> <?php echo $form_styling; ?> action="index.php" method="post" name="<?php echo $unique_form_id?>" id="<?php echo $unique_form_id?>" enctype="multipart/form-data">
    <?php
...
//var_dump($field_from_params);
    ...
    echo '<div '.$div_styling.'>';
      ...
      <input <?php echo $field_styling; ?> type="button" name="request_button" value="<?php echo $buttonText; ?>"
                   class="button <?php echo $custom_class; ?>" <?php echo $offset_animation; ?> onclick="javascript:sendRequest<?php echo $layout->type?>('send_request',<?php echo $unique_form_id?>);">
    <?php
    echo '</div>';
    ...
    <input type="hidden" name="fk_eiid" value="<?php echo $parent_eiid ?>"/>
