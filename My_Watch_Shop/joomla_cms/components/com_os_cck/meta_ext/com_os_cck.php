 <?php
/**
 * shCustomTitleTag support for com_os_cck component.
 *
 * @author      $Author:  $
 * @copyright   
 * @package    
 ...
 *
 * Some parts from:
 * 404SEFx support for com_os_cck component.
 *
 */
...
class shSEFConfig_1 {
  ...
  var $shSecCheckPOSTData = true;    // if set to yes, POST data will not be checked for mosconfig, script, base_64,
...
$database = JFactory::getDBO();
$menuId = JREQUEST::getInt('Itemid', null);
$view = JREQUEST::getCmd('view', null);
$catid = JREQUEST::getInt('catid', null);
$eiid = JREQUEST::getInt('eiid', null);
$lid = JREQUEST::getInt('lid', null);
...
$text_fields = array();
// if(multiLanguage)
// $shLangIso = shLoadPluginLanguage( 'com_os_cck', $shLangIso, 'COM_SH404SEF_CREATE_NEW');
...
switch ($view) {
  case 'all_instance':
    $toGet = 'all_instance_layout';
    break;
  case 'calendar':
    $toGet = 'calendar_layout';
    break;
  case 'all_categories':
    $toGet = 'allcategories_layout';
    break;
  case 'add_instance':
    $toGet = 'request_layout';
    break;
  case 'show_search':
    $toGet = 'search_layout';
    break;
...
if($toGet){
  if(!isset($lid)) $lid = JFactory::getApplication()->getMenu()->getItem($menuId)->params->get($toGet);
  $title[] =  JFactory::getApplication()->getMenu()->getItem($menuId)->title;
}


if($view == 'category'){
  if(!isset($catid)) $catid = JFactory::getApplication()->getMenu()->getItem($menuId)->params->get('category');
  if(!isset($lid)) $lid = JFactory::getApplication()->getMenu()->getItem($menuId)->params->get('category_layout');
}
...
if($view == 'instance'){
  ...
  if(!isset($eiid[0])){
    $eiid = array();
    $eiid[0] = JFactory::getApplication()->getMenu()->getItem($menuId)->params->get('instance');
  }   

  if(!isset($lid)){
    $lid = JFactory::getApplication()->getMenu()->getItem($menuId)->params->get('instance_layout');
  } 

  if(!empty($eiid[0])){
    $query = "SELECT con.fk_cid AS catid FROM #__os_cck_categories_connect AS con ".
    " LEFT JOIN #__os_cck_entity_instance as c ON c.eiid = " . $eiid[0];
...
if(isset($lid)){
	$query = "SELECT title FROM #__os_cck_layout
      ...
      $database->setQuery($query);
      $layout_title = $database->loadResult();
      if($layout_title && !empty($layout_title)){
      	$title[] = $layout_title;
      }else{
      	$title[] = '';
      }
...
if(isset($catid)) {
        ...
        $query = "SELECT c.cid, c.title, c.parent_id, c.description FROM #__os_cck_categories AS c
        ...
        if(isset($row)) {
          ...
          $desctitle[] = strip_tags($row->description);
       
          while(isset($row) && $row->parent_id>0) {
            $query = "SELECT  c.title, c.cid AS catid, parent_id, c.description  
...
if(isset($eiid[0])) {
      ...
      $text_fields = array_slice($text_fields,0,5);
      $text_fields = implode(', ', $text_fields);

      if(!empty($title_field)){
        $query = "SELECT ".$text_fields." FROM #__os_cck_content_entity_".$fk_eid."
...
// $cck = "OS CCK";
//To set Title
$shCustomTitleTag = mylenStr($tagtitle, 75);
//To set meta Description
$shCustomDescriptionTag = mylenStr($tagtitle." | ".$desctitle, 200);
//To set meta KeywordsTag
$shCustomKeywordsTag = mylenStr($tagtitle, 250);
