...
//$counter_calc_price = (count($layout_params['calculated_price_fields']))? max(array_keys($layout_params['calculated_price_fields']))+1 : 1 ;

$counter_calc_price = (isset($layout_params['calculated_price_fields']) && !empty($layout_params['calculated_price_fields']))? max(array_keys($layout_params['calculated_price_fields']))+1 : 1 ;
...
$total_price->label = JText::_("COM_OS_CCK_CART_UNIQUE_FIELD_TOTAL_PRICE");
$total_price->db_field_name = 'total_price';
$total_price->field_type = 'total_price';
$total_price->description = 'Total price';

$unique_fields = array($cck_send_button, $coupon, $total_price);
          ...
          <div>
            ...
            <span class="field-name <?php echo $cck_calculated_price->db_field_name.'-'.$counter_calc_price.'-label-hidden';
                        echo  (isset($fields_from_params['showName_'.$cck_calculated_price->db_field_name.'_'.$counter_calc_price])
                                || !$layout->lid)?
                               '':
                               ' hide-field-name-'.$cck_calculated_price->db_field_name.'_'.$counter_calc_price;?>"
...
<?php
        ...
        for ($i = 0; $i < count($fields); $i++) {
          $field = $fields[$i];
          if(stripos($field->db_field_name, 'price') !== FALSE)
                  continue;
          //skip child selects
        ...
        <div id="options-tab-1">
          <div id="fields-options-accordion">
            ...
            <div class="main-fields-content main-fields-options"> 
              <?php
              foreach ($unique_fields as $field) {
                //require getLayoutPathCCK::getAdminLayoutViewPath('com_os_cck', 'buy_request_instance_layout', 'fieldUniq');
