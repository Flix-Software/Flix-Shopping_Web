...
if (!empty($entityInstance->eiid)) {
    $db->setQuery("SELECT * FROM #__os_cck_audio_source WHERE fk_eiid=" . $entityInstance->eiid . " ORDER BY 'id'");
    ...
    newitem = "<div>"+
                  ...
                  "<span width='85%'>"+
                    "<input style=\"float:left; width:100%\" type=\"file\" "+
                      " onClick=\"jQuerOs('#new_upload_audio_url"+upload_audios+"').val('');\" " +
...
if (count($audios) > 0) {?>
  ...
  <div>
    ...
    <?php
    for ($k = 0;$k < count($audios);$k++) { ?>
      <div>
        <span align="right"><?php echo JText::_("COM_OS_CCK_LABEL_AUDIO_ATTRIBUTE").($k + 1)?></span>
        <span>
          <?php
          if (isset($audios[$k]->src) && substr($audios[$k]->src, 0, 4) != "http") {?>
            <input class="audio_src" type="text" size="60" value="<?php echo JURI::root() . $audios[$k]->src?>" readonly="readonly"/>
            <?php
          }else{?>
            <input class="audio_src" type="text" size="60" value="<?php echo $audios[$k]->src?>" readonly="readonly"/>
            <?php
          }?>
        </span>
      </div>
      <div>
        ...
        <span>
          <?php
          if(isset($audios[$k]->id)){?>
            <input type="checkbox" name="audio_option_del<?php echo $audios[$k]->id?>" value="<?php echo $audios[$k]->id?>">
            <?php
          }?>
        </span>
      </div>
...
rel="tooltip" data-toggle="tooltip" data-placement="top" title="<?php echo $layout_params['fields'][$fName.'_tooltip'];?>"
    <?php } ?> <?php echo $span_style; ?>>
  <span id="a_items">
      <input <?php echo $layout_params['field_styling']?> class="<?php echo $layout_params['custom_class'].$required . $field->db_field_name?>"  <?php echo $offset_animation; ?> type="button" name="new_audio"
