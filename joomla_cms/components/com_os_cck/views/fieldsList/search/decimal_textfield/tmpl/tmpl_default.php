...
if(stripos($fName, 'e-p-ch') !== false){
    $child_field = new os_cckEntityField($db);
    $child_field->load($field->fid);
    $db_field_name = $child_field->db_field_name;
}else{
    $db_field_name = $fName;
}

//var_dump($field); exit;
$query = "SELECT ent.".$db_field_name." FROM #__os_cck_content_entity_".$field->fk_eid." as ent"
    ...
    jQuerOs(function() {
        ...
        jQuerOs("input#os_cck_search_<?php echo $fName.$i?>_from").change(function(){
            ...
            if(parseInt(value1)<(<?php echo (int) $min_number;?>))
                {
                    jQuerOs("input#os_cck_search_<?php echo $fName.$i?>_from").val(<?php echo (int) $min_number;?>);
                }
        ...
        jQuerOs("input#os_cck_search_<?php echo $fName.$i?>_to").change(function(){
            ...
            if (value2 > '<?php echo $max_number?>') {
                ...
                jQuerOs("input#os_cck_search_<?php echo $fName.$i?>_to").val('<?php echo $max_number?>')
            }
