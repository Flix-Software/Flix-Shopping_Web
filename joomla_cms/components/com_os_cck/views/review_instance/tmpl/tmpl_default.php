...
<div class="cck-body">
  ...
  if($show_type == 2){
     ...
     $offset_animation = isset($button_style['offset_animation']) ? $button_style['offset_animation'] : '';
      ?>
  <div <?php echo $div_styling; ?>>
    <input <?php echo $field_styling;?>  type="button" class="button btn btn-info <?php echo $custom_class;?>" <?php echo $offset_animation; ?> value="<?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_REVIEW_REQUEST'); ?>"
  ...
  <form class="<?php echo $form_custom_class; ?>" <?php echo $form_offset_animation; ?> <?php echo $form_hover_animation; ?> <?php echo $form_styling; ?> action="index.php" method="post" name="<?php echo $unique_form_id?>" id="<?php echo $unique_form_id?>" enctype="multipart/form-data">
      ...
      if(strpos($layout_html,"{|f-".$field->fid."|}")){
          ...
          if($php_result){
            ...
            if(isset($field_from_params['access_'.$field->db_field_name])
              ...
              $user = JFactory::getUser();
              if(!checkAccess_cck($field_from_params['access_'.$field->db_field_name], $user->groups, $field->fk_eid, 'fields')){
    ...
    echo '<div '.$div_styling.'>';
      ...
      <input <?php echo $field_styling; ?> type="button" name="review_button" value="<?php echo $buttonText; ?>"
                      class="button <?php echo $custom_class; ?>" <?php echo $offset_animation; ?> onclick="javascript:sendRequest<?php echo $layout->type?>('send_review_request',<?php echo $unique_form_id?>);">
      <?php
      echo '</div>';
