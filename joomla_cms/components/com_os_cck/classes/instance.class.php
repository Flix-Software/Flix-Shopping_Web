          ...
          for ($j = 1;isset($_FILES[$field->fid . '_new_upload_video' . $j]) 
            ...
            || array_key_exists($field->fid . '_new_upload_video_youtube_code' . $j, $_POST);$j++) { 
              ...
              if (isset($_FILES[$field->fid . '_new_upload_video' . $j]) && $_FILES[$field->fid . '_new_upload_video' . $j]['name'] != "") {
                //storing e-Document
                $video = JRequest::getVar($field->fid . '_new_upload_video' . $j, '', 'files');
              ...
              if (array_key_exists($field->fid . '_new_upload_video_url' . $j, $_POST) && $_POST[$field->fid . '_new_upload_video_url' . $j] != "") {
                ...
                $uploadVideoURL = JRequest::getVar($field->fid . '_new_upload_video_url' . $j, '', 'post');
              ...
              if (array_key_exists($field->fid . '_new_upload_video_youtube_code' . $j, $_POST) 
                && $_POST[$field->fid . '_new_upload_video_youtube_code' . $j] != "") {
                  $uploadVideoYoutubeCode = JRequest::getVar($field->fid . '_new_upload_video_youtube_code' . $j, '', 'post');
  ...
  function getFieldValue($field, $multiple_num = 0){
    ...
    if ($field->field_type == 'videofield') {
      ...
      $result = $this->_db->loadObjectList();
        ...
        $query = "SELECT * FROM #__os_cck_content_instances_price WHERE fk_fid='$field->fid' AND fk_eiid='$this->eiid'";
        $this->_db->setQuery($query);
        $result = $this->_db->loadObjectList();
  ...
  function accept(){
    ...
    if (isset($lendTerm[0])) {
      for ($e = 0, $m = count($lendTerm); $e < $m; $e++) {
          $rent_check = checkRentCCK($lendTerm[$e]->rent_from, $lendTerm[$e]->rent_until, $lend_from, $lend_until);
        //rent check
...
//        }
          if($rent_check !== NULL){
              echo "<script> alert('" . $rent_check . " '); window.history.go(-1); </script>\n";
            exit ();
          }
      }
    }
