...
class Category
{
  static function listCategories($option, $lid = 0){  
    ...
    $query = "SELECT c.*,(SELECT COUNT(ccc.fk_cid) FROM #__os_cck_categories_connect AS ccc "
                            ...
                            . "\n AND c.published = '1' $custom_sql_where "
                            . "\n ORDER BY c.ordering ";
    $db->setQuery($query);
    ...
    for ($i = 0; $i < count($cat_all); $i++) {
      if (os_cck_site_controller::is_exist_curr_and_subcategory_items($cat_all[$i]->cid, $allcategories_layout->fk_eid )) {
        $cat_all[$i]->display = 1;
        if($fields_from_params['views']['sub_category_level'] == 0 && $cat_all[$i]->parent_id > 0 && isset($cat_all[$i]->display))
            $cat_all[$i]->display = 0;
      }
    }
  ...
  static function showCategory($option, $catid, $lid,$parent_layout_params = 0, $mod_type = 0){ 

    global $app, $db, $acl, $user, $doc, $session, $cck_entity_configuration;
...
if($_REQUEST['option'] == 'com_os_cck'){
    ...
    if($mod_type == 0){
        ...
        $fields_from_params["views"]["sortType_category_order_by"] = isset($fields_from_params["fields"]["sortType_category_order_by"]) ? $fields_from_params["fields"]["sortType_category_order_by"] : '';
    }
...
//var_dump($mod_type);
    ...
    if(isset($layout_params['views']['subcategories_layout']) && $layout_params['views']['subcategories_layout'] == -1){
        ...
        $layout_params['views']['subcategories_layout'] = $layout->getDefaultLayout($entity_id, 'category');
