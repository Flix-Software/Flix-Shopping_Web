...
if ($this->config->getSocial()->getSharingLinkType() == 'event') {
	$this->link =  JRoute::_( 'index.php?option=com_eventgallery&view=event&folder='.$file->getFolderName(), false, \Joomla\CMS\Router\Route::TLS_IGNORE, true);
} else {
	$this->link =  JRoute::_( 'index.php?option=com_eventgallery&view=singleimage&layout=minipage&folder='.$file->getFolderName().'&file='.$file->getFileName(), false, \Joomla\CMS\Router\Route::TLS_IGNORE, true );
    if (null != $articleUrl) {
